...
def save_model(output_dir, epoch, model, model_without_ddp, optimizer, loss_scaler, name = ""):
...
def load_model(name, model_without_ddp, optimizer, loss_scaler):
...
def adjust_learning_rate(optimizer, epoch, lr, total_epochs, warmup_epochs):
    """Decay the learning rate with half-cycle cosine after warmup"""
    if epoch < warmup_epochs: # warmup epochs. The lr increases until the warmup
        lr = lr * epoch / warmup_epochs
    else:
        lr = lr * 0.5 * (1. + math.cos(math.pi * (epoch - warmup_epochs) / (total_epochs - warmup_epochs)))
